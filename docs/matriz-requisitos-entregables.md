...
| §6 Panel de indicadores (deseable) | Panel que aborde al menos un síntoma identificado por ECON: tiempos muertos, baja trazabilidad o ausencia de indicadores unificados. | Página `/indicadores` («Indicadores y SLA», `apps/api/app/dashboard/indicator_views.py`) sobre `GET /api/v1/indicators` (`apps/api/app/api/indicators.py`, `apps/api/app/services/indicators.py`): ocho fichas agrupadas por área con lectura en lenguaje llano, filas, cobertura y SLA propuesto al lado; página `/decisiones` (`views.py::decisions`) con un asunto por solicitud, las tres lecturas más accionables de los indicadores, período de uso solicitado y distribución por estado (`decision_priorities.py`, `decision_analytics.py`); fichas y SLA en `docs/indicadores-calculables.md`; pruebas `apps/api/tests/test_indicator_views.py` y `test_decision_priorities.py`. | `/indicadores?mode=fixture`; `/decisiones?mode=fixture`; `GET /api/v1/indicators?mode=fixture`. | cumple | Con la muestra la mayoría de las fichas queda «no evaluable» con motivo (sin corte, sin tareas) y ninguna publica promedios: el panel muestra cobertura, no desempeño. Acción: en `live` completar el registro antes de leerlo como serie. |
