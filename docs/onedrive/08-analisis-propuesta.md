...
La introducción asigna a **Equipo 6 — Los Filósofos** la retroexcavadora **RE-03**, el motorista de sandbox **MOT-006 — Rodrigo Trujillo**, el proyecto **PROY-006 — Proyecto Zeta** y la geocerca relacionada **Proyecto Zeta — San Salvador**. La referencia a Rodrigo como motorista no demuestra que sea un integrante del equipo participante. [Introducción, apartado «Tu equipo y recursos asignados»](00-introduccion-equipo.md#hub-de-operaciones).
...
| `horometro` forma parte del evento de geocerca. | El único campo documentado está en **Mantenimiento** de Startrack, tipo `numero/h`. No hay evidencia de que viaje en un webhook GPS. [Startrack fila 40](06-diccionario-de-datos/03-startrack.md#fila-40). | Registrar su procedencia, unidad y momento de medición. No calcular utilización diaria a partir de una sola lectura ni confundirla con odómetro. |
...
| RF-04: consulta unificada | Persistencia y consulta navegable de datos relacionados, no solo el activo fijo del diccionario. | Alta |
