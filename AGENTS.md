...
The UI library is dash-mantine-components (Mantine v8) with local
Tabler icons (`app/dashboard/assets/icons`) and AG Grid Community tables; register pages in `PAGES`
...
and primary actions only: blue is brand, not state. Icons are local Tabler SVGs (add the file before using a new name); never make an icon the only carrier of meaning.
