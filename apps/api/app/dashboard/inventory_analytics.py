...
from app.dashboard.theme import LINE, figure, state_color
...
def inventory_status_panel(
    ...
    chart = figure(height=max(190, 46 * len(ordered) + 88))
    chart.add_trace(
        go.Bar(
    ...
    chart.update_xaxes(
        title_text="Equipos de esta lectura",
        title_standoff=12,
        range=[0, axis_total],
        tick0=0,
    ...
    chart.update_layout(showlegend=False, margin={"l": 8, "r": 48, "t": 8, "b": 48})
    scope = (
    ...
    return section(
        ...
        hint(
            f"{scope} El estado administrativo es el valor de Prisma: no confirma "
            "disponibilidad física ni describe la flota completa.",
            className="page-note",
        ),
        ...
        className="page-section inventory-panel",
    )
