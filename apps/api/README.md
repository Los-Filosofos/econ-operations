...
The homepage (`/`) shows requested usage periods with assignment context, followed by operational decisions and evidence. The Mantine shell keeps an ECON blue sidebar and the header available. Indicators show measured durations with visible supporting records; proposed SLAs remain reference tables. Integration opens on the four-stage trace and its unresolved inputs, while Sources charts read coverage without inventing missing totals. Dedicated pages include requests (with candidate units for a pending request without unit), machinery, operations workflow (paginated registry), integration trace (`/integracion`), indicator sheets (`/indicadores`), decisions (`/decisiones`, an alias of home), sources and admin user management (`/administracion`). There is no refresh button: the page polls `GET /api/v1/status` every 15 s and reloads data only when `registry_version` changes.
